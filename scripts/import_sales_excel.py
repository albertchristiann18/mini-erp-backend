...
# Config
# ---------------------------------------------------------------------------
EXCEL_PATH = "/Users/jtf01644/personal/mini-erp-project/mirako_data/Sales Order Tracker.xlsx"
...
def main():
    ...
    with psycopg.connect(**DB_PARAMS, autocommit=True) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT warehouse_id FROM master_warehouse "
            ...
            cur.execute(
                ...
                "  SELECT product_variant_id FROM master_productvariant "
                "  WHERE company_id = %s"
                ")",
                (company_id,),
            )
            ...
            cur.execute(
                ...
                "            master_productvariant.product_variant_id"
                "    ) "
                "WHERE company_id = %s",
                (company_id,),
            )
